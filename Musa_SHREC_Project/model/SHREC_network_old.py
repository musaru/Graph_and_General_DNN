import os
os.chdir('/home/musa/Musa_Related/PhD/Skelton_Depth_VedioSign Language/Depth_Skelton/Depththreedatasetcode/SHREC_Dataset_DG-STA/')
print(os.getcwd())
from model.SHREC_st_att_layer import *
from model.SHREC_st_att_layer import LayerNorm
...
class DG_STA(nn.Module):
    ...
    def forward(self, x):
        ...
        #first
        #input map
        #nmp=res=self.res_map(original)
        inmp = self.input_map1(original)  #input=32*176*3
        ...
        sx=self.res_map(original)+sx
        tx = self.t_att(sx) #
        #tx1=self.res_map(original)+tx
        tx1=self.input_map(original)+tx
        
        
        #2nd Stage
        #first
        #input map
        inmp=res=self.res_map(original)
        #inmp = self.input_map(original)  #input=32*176*3
        #print('input map of first attention',inmp.shape)     #output 32*176*128
        #spatal
        ...
        sx=self.res_map(original)+sx
        tx = self.t_att(sx) #
        #tx2=self.res_map(original)+tx
        tx2=self.input_map(original)+tx
        #print('Second attention',tx.shape) 
        com=tx1+tx2+self.input_map1(original)
        #com=tx1+tx2
        #print(com.shape)
        x = com.sum(1) / com.shape[1]
        pred = self.cls(x)
