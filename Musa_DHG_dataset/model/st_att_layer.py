...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, ft_size, time_len, domain):
        ...
        if domain == "temporal" or domain == "mask_t":
            ...
            #print('Temporal position list', pos_list)
            ...
            #print('Spatial position list', pos_list)
        
        position = torch.from_numpy(np.array(pos_list)).unsqueeze(1).float()
        #print('unsequeez position', position)

        # Compute the positional encodings once in log space.
        pe = torch.zeros(self.time_len * self.joint_num, ft_size) #175*128
...
class ST_ATT_Layer(nn.Module):
    ...
    def __init__(self, input_size, output_size, h_num, h_dim, dp_rate, time_len, domain):
        ...
        self.ft_map = nn.Sequential(
                        ...
                        #nn.Linear(output_size, output_size), #New
                        #nn.ReLU(),          #New
                        nn.Dropout(dp_rate)
