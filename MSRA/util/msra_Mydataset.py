...
class Hand_Dataset(Dataset):
    ...
    def __getitem__(self, ind):   # for training 0 to 2660, for test 0 to 140
        ...
        #print('before agumention', skeleton.shape)  #46*22*3
        if self.use_data_aug:
            skeleton = self.data_aug(skeleton)  # send the skeleton to augmention
        #print('After Skeleton agumention', skeleton.shape)  #45*22*3
        # sample time_len frames from whole video
        data_num = skeleton.shape[0]  #data_num=45
        #print("mydataset _getitem_ data_name",data_num)
        
        idx_list = self.sample_frame(data_num) 
        #print('idx_list', idx_list)
        skeleton = [skeleton[idx] for idx in idx_list]
        skeleton = np.array(skeleton)
        #print('After idex agumention', skeleton.shape)
        
        #print("mydataset _getitem_ Skeleton shape",skeleton.shape)
    ...
    def sample_frame(self, data_num):
        ...
        while len(idx_list) < sample_size:
            ...
            if idx not in idx_list:
                idx_list.append(idx)
        idx_list.sort()
        #print('index list in sample_frame', idx_list)
